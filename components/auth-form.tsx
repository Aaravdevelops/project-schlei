...
export function AuthForm({ mode }: { mode: 'sign-in' | 'sign-up' }) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); setBusy(true); setError(''); const data = new FormData(e.currentTarget); const result = mode === 'sign-up' ? await authClient.signUp.email({ name: String(data.get('name')), email: String(data.get('email')), password: String(data.get('password')) }) : await authClient.signIn.email({ email: String(data.get('email')), password: String(data.get('password')) }); if (result.error) setError('We could not complete that request. Check your details and try again.'); else { router.push('/dashboard'); router.refresh() }; setBusy(false) }
