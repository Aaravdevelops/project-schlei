...
export async function readRssFeeds(): Promise<{ feeds: RssFeedResult[]; updates: RssItem[] }> {
  ...
  const results = await Promise.all(configured.map(async (url) => { const name = new URL(url).hostname; try { const response = await fetch(url, { signal: AbortSignal.timeout(timeoutMs), headers: { accept: 'application/rss+xml, application/atom+xml, application/xml, text/xml' }, cache: 'no-store' }); if (!response.ok) throw new Error(`HTTP ${response.status}`); const parsed = items(await response.text()); if (!parsed.length) return { name, status: 'error' as const, lastChecked: now, detail: 'Feed returned no valid publication entries.', items: [] }; const fetchedAt = now; return { name, status: 'available' as const, lastChecked: now, detail: `${parsed.length} publication${parsed.length === 1 ? '' : 's'} available.`, items: parsed.map((item) => ({ ...item, id: item.link, source: name, fetchedAt, sourceType: 'rss' as const })) } } catch (error) { return { name, status: 'error' as const, lastChecked: now, detail: error instanceof Error ? error.message : 'Feed could not be read.', items: [] } } }))
